...
The palette intentionally uses only vanilla 1.12.2 block IDs so the schematic
stays stable after a world wipe or a Forge registry rebuild.
"""
...
# Stable vanilla substitutions for the older modded palette names. Keeping the
# semantic names below makes the build code read like the intended architecture
# while avoiding Forge numeric IDs from a previous world registry.
TC_STONE_ARCANE = STONE_BRICK
TC_STONE_ARCANE_BRICK = STONE_BRICK
TC_STONE_ANCIENT = COBBLE
TC_STONE_ANCIENT_TILE = STONE_BRICK
TC_STONE_ELDRITCH_TILE = NETHER_BRICK
TC_LOG_GREATWOOD = LOG
TC_LOG_SILVERWOOD = LOG
TC_PLANK_GREATWOOD = PLANKS
TC_PLANK_SILVERWOOD = PLANKS
TC_PAVING_TRAVEL = STONE_BRICK
TC_PAVING_BARRIER = STONE_BRICK
TC_PILLAR_ARCANE = QUARTZ
TC_LAMP_ARCANE = SEA_LANTERN

DRPG_BLOODGEM_BLOCK = GOLD
DRPG_ARLEMITE_BRICKS = STONE_BRICK
DRPG_DARKSTONE_BRICKS = NETHER_BRICK
DRPG_EDEN_BRICKS = QUARTZ
DRPG_WILDWOOD_BRICKS = PLANKS
DRPG_APALACHIA_BRICKS = STONE_BRICK
DRPG_SKYTHERN_BRICKS = LAPIS
DRPG_MORTUM_BRICKS = NETHER_BRICK
DRPG_ARLEMITE_LAMP = GLOWSTONE
DRPG_BLUEFIRE_LAMP = SEA_LANTERN
DRPG_DIVINE_LAMP = GLOWSTONE
DRPG_DARKSTONE = NETHER_BRICK
DRPG_DIVINE_ROCK = QUARTZ
DRPG_ANCIENT_BRICKS = STONE_BRICK
DRPG_ANCIENT_STONE = COBBLE
DRPG_ANCIENT_TILE = STONE_BRICK
