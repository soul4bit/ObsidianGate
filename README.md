...
Для защиты инвентарей сервер дополнительно запускает `mc-rpg-playerdata-backup.timer`: каждые 15 минут архивируются `world/playerdata`, `world/stats` и `world/advancements` в `~/mc-rpg/backups/playerdata/`. Хранится 96 последних архивов. Если у игрока пропал инвентарь после смены ника или UUID, сначала проверьте соответствующий `.dat` в этих архивах и восстанавливайте только файл игрока, не весь мир.
